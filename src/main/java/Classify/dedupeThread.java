...
"      \"must\": [\n" +
"        {\n" +
"                   \"match\": {\"imageURL.imgURL.keyword\":\"" + tkey.replace("\"", "\\\"") + "\"}\n" +
"        }\n" +
"      ],\n" +
"      \"must_not\": [\n" +
"        {\"match\": {\n" +
...
"  \"scroll_id\":\"$SCROLL\"}}";
    ...
    private void htAddArray(HashSet<String> hs, JSONArray jar){
        for(int i=0; i< jar.size(); i++){
            if(!hs.contains(jar.get(i)))
                hs.add((String)jar.get(i));
        }
    }
...
"{\"doc\":{\"isduplicate\":true}}\n";
    ...
    public static String mkMasterString(String indexname, String id, HashSet<String> hs, String pname){
        ...
        msb.append("]");
        String template = "{\"update\":{\"_id\":\"$ID\", \"_type\":\"product\", \"_index\":\"$INDEX\"}}\n" +
"{\"doc\":{ \"isduplicate\":false, \"searchPname\":$PNAME}}\n";
